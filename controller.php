...
{
    return (strlen($password) >= 10 && strlen($password) <= 64 && preg_match(
        "/[a-z]/",
        $password
    ) && preg_match(
        "/[A-Z]/",
    ...
    if (mysqli_num_rows($code_res) > 0) {
        ...
        if ($current_time - $otp_time <= $otp_valid_duration) {
            ...
            if ($update_res) {
                ...
                $errors['otp-error'] = "Failed to update OTP status.";
            }
        } else {
            $errors['otp-error'] = "OTP has expired.";
        }
    } else {
...
if (isset($_POST['check-email'])) {
    ...
    if (mysqli_num_rows($run_sql) > 0) {
        $code = rand(999999, 111111);
        $otp_time = time();
        $insert_code = "UPDATE users SET code = $code, otp_time = $otp_time WHERE email = '$email'";
